...
func TestCreateNode(t *testing.T) {
	t.Run("can create node", func(t *testing.T) {
		...
		err := nodeService.Create(&domain.Node{
			Hostname: "node1.cluster.com",
			Port:     "8080",
			ShardID:  1,
		})
		...
		if node.ShardID != 1 {
			t.Errorf("Expected shard ID to be 1, got %v", node.ShardID)
		}
	})

	t.Run("cannot create node with missing hostname", func(t *testing.T) {
		nodeRepo := nodeMemRepo.NewMemoryRepository()
		nodeService := service.NewNodeService(nodeRepo)
		...
		if err.Error() != "hostname and port are required" {
			t.Errorf("Expected error to be 'hostname and port are required', got %v", err.Error())
		}
	})
}
